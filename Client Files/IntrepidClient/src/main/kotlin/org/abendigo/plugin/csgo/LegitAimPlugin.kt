...
object LegitAimPlugin : InGamePlugin(name = "Aim Assist Legit", duration = 25) {
	...
	private fun aimAt(position: Vector, angle: Vector, target: Player, unlockFOV: Float) {
		...
		if (deltaFOV >= unlockFOV) LegitAimPlugin.target = null
		else angleSmooth(aim, angle, smoothing)

		if (NEVER_STICK) LegitAimPlugin.target = null
	}
